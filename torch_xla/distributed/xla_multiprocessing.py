...
TfDevice = collections.namedtuple('TfDevice',
                                  'job replica task dev_kind dev_index fqdev')
...
def _create_xla_local_device_name(kind, index, task):
  return '/job:{}/replica:0/task:{}/device:{}:{}'.format(
...
def _parse_device_config(config, select=None):
  ...
  for devcfg in config.split('|'):
    ...
    if select is None or re.match(select, m.group(1)):
...
def _generate_device_config(kind, num_devices, dev_index=None):
  devices = collections.OrderedDict()
  for index in range(0, num_devices):
    devstr = _create_xla_local_device_name(
        'XLA_{}'.format(kind), dev_index if dev_index is not None else index,
        index)
    tfdev = _parse_tf_device(devstr)
    devices['{}:{}'.format(kind, index)] = Device(
        kind=kind, index=index, tfdev=tfdev)
  return devices
...
def _parse_gpu_env_config():
  ...
  if num_gpus is not None:
    ...
    # to 1, and its device index is always 0.
    return _generate_device_config(
        'GPU', int(os.environ[xenv.WORLD_SIZE]), dev_index=0)
...
def _get_devices_per_worker():
  ...
  devmap = os.environ.get(xenv.DEVICE_MAP, None)
  assert devmap, 'Missing device configuration'
  devices = _parse_device_config(devmap, select=r'GPU|TPU')
  assert devices, 'Missing device configuration'
  return 1, list(devices.values())[0].kind
...
def _pre_fork_setup(num_devices):
  ...
  if dev_kind == 'GPU' and _NCCL_COMM_ID not in os.environ:
...
def _setup_gpu_worker(index, gindex, pf_cfg):
  ...
  for i, device in enumerate(gpu_devices.values()):
    ...
    devices.append('{}:{};{}'.format(device.kind, device.index,
                                     device.tfdev.fqdev))
